...
export default function TokenDetailPage() {
  ...
  async function handleRefreshData() {
    try {
      ...
      setRefreshMessage('Running backend analytics pipeline...');

      await refreshTokenPipeline(symbol);
      await loadTokenDashboard();

      setRefreshStatus('success');
      setRefreshMessage('Pipeline refreshed successfully.');
      ...
      setRefreshMessage(
        error?.response?.data?.error?.message || 'Failed to refresh token pipeline.'
      );
    }
  }
  ...
  if (status === 'error') {
    return <ErrorState message="Failed to load token detail" />;
  }
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="grid gap-8 xl:grid-cols-[1.35fr_0.65fr] xl:items-center">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3 text-xs text-slate-500">
              ...
              <span className="rounded-full bg-slate-900 px-3 py-1">
                Score version: {overview.scoreVersion || '—'}
              </span>
            </div>
          </div>
      ...
      <section className="grid gap-4 lg:grid-cols-4">
        <MetricCard
          label="CEX Netflow 7d"
          value={formatNumber(overview.cex?.netflow7d)}
          hint={formatUsd(overview.cex?.netflowUsd7d)}
          signed
        />
        ...
        <MetricCard
          ...
          hint="Tokens entering exchanges"
        />

        <MetricCard
          label="CEX Outflow 7d"
          value={formatNumber(overview.cex?.outflow7d)}
          hint="Tokens leaving exchanges"
        />

        <MetricCard
          label="Non-CEX Holder Change"
          value={formatNumber(overview.holders?.nonCexBalanceChange7d)}
          hint="Top holders excluding CEX"
          signed
        />
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Card
          title="CEX flow structure"
          subtitle="Daily netflow breakdown. Negative values mean supply is leaving known CEX wallets."
        >
          {cexChartData.length ? (
            <div className="h-80">
                ...
                {holders.items.map((holder) => (
                  <tr key={holder.id} className="table-row">
                    ...
                    <td className="px-4 py-3">
                      {holder.address.label || holder.address.entity?.name || 'Unknown'}
                    </td>
                    <td className="px-4 py-3">
                      <Badge
                        className={
